...
void score_links_and_remove_roads() {
	std::ranges::for_each(
			...
			[](m2::Character& player) {
				...
				player.SetVariable(pb::VICTORY_POINTS, m2::IVFE{player.GetVariable(pb::VICTORY_POINTS).GetIntOrZero() + linkCount});
			});
	RemoveAllRoads();
}
...
void score_sold_factories_and_remove_obsolete() {
	std::ranges::for_each(
			...
			[](const m2::Character& factory) {
				...
				player.SetVariable(pb::VICTORY_POINTS, m2::IVFE{player.GetVariable(pb::VICTORY_POINTS).GetIntOrZero() + victoryPointsBonus});
			});
