...
DevelopJourney::~DevelopJourney() {
	...
	if (_reserved_source_1) {
		_reserved_source_1->GetCharacter().UnsafeSetVariable(IRON_CUBE_COUNT, IVFE{_reserved_source_1->GetCharacter().GetVariable(IRON_CUBE_COUNT).GetIntOrZero() + 1});
		_reserved_source_1 = nullptr;
	}
	if (_reserved_source_2) {
		_reserved_source_2->GetCharacter().UnsafeSetVariable(IRON_CUBE_COUNT, IVFE{_reserved_source_2->GetCharacter().GetVariable(IRON_CUBE_COUNT).GetIntOrZero() + 1});
...
m2::void_expected CanPlayerDevelop(m2::Character& player, const m2g::pb::TurnBasedClientCommand_DevelopAction& develop_action) {
	...
	if (reserved_resource) {
		reserved_resource->GetCharacter().UnsafeSetVariable(IRON_CUBE_COUNT, IVFE{reserved_resource->GetCharacter().GetVariable(IRON_CUBE_COUNT).GetIntOrZero() + 1});
	}
