...
NetworkJourney::~NetworkJourney() {
	...
	for (auto& source : _resource_sources) {
		if (source.reserved_object) {
			source.reserved_object->GetCharacter().SetVariable(source.resource_type, IVFE{source.reserved_object->GetCharacter().GetVariable(source.resource_type).GetIntOrZero() + 1});
			source.reserved_object = nullptr;
		}
	}
	if (_decoy_road_1) {
		m2::CreateObjectDeleter(_decoy_road_1)();
		_decoy_road_1 = 0;
	}
	if (_decoy_road_2) {
		m2::CreateObjectDeleter(_decoy_road_2)();
		_decoy_road_2 = 0;
	}
}
...
std::optional<NetworkJourneyStep> NetworkJourney::HandleResourceEnterSignal() {
	...
	if (auto unspecified_resource = GetNextUnspecifiedResource(); unspecified_resource != _resource_sources.end()) {
		...
		if (unspecified_resource->connection == _selected_connection_1 && not _decoy_road_1) {
			...
			auto it = m2::CreateObject(m2g::pb::ROAD, M2_PLAYER.GetId());
			InitRoad(*it, position_of_connection(_selected_connection_1), _selected_connection_1);
			_decoy_road_1 = it->GetId();
		}
		if (unspecified_resource->connection == _selected_connection_2 && not _decoy_road_2) {
			// Create the second decoy
			auto it = m2::CreateObject(m2g::pb::ROAD, M2_PLAYER.GetId());
			InitRoad(*it, position_of_connection(_selected_connection_2), _selected_connection_2);
			_decoy_road_2 = it->GetId();
		}
	...
	if (s.poi()) {
		...
		if (auto *factory = FindFactoryAtLocation(selected_location)) {
			...
			if (M2_LEVEL.GetDimmingExceptions()->contains(factory->GetId())) {
				// Reserve resource
				factory->GetCharacter().SetVariable(unspecified_resource->resource_type, IVFE{std::max(factory->GetCharacter().GetVariable(unspecified_resource->resource_type).GetIntOrZero() - 1, 0)});
				unspecified_resource->reserved_object = factory;
