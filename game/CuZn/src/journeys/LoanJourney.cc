...
m2g::pb::CardType ExecuteLoanAction(m2::Character& player, const TurnBasedClientCommand_LoanAction& loan_action) {
	...
	player.UnsafeSetVariable(INCOME_POINTS, VariableValue{ClampIncomePoints(newIncomePoints)});
	player.UnsafeSetVariable(MONEY, VariableValue{player.GetVariable(MONEY).GetIntOrZero() + 30});
