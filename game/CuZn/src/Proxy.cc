...
void m2g::Proxy::turnBasedServerPopulate(MAYBE const std::string& name, MAYBE const m2::pb::Level& level) {
	...
	// Assign licenses to active merchants
	{
		...
		for (const auto& possibly_active_merchant_loc : possibly_active_merchant_locs) {
			...
			if (license != pb::NO_MERCHANT_LICENSE) {
				...
				merchant_char.UnsafeSetVariable(pb::BEER_BARREL_COUNT, IVFE{merchant_char.GetVariable(pb::BEER_BARREL_COUNT).GetIntOrZero() + 1});
			}
		}
	}
	...
	game_state_tracker().UnsafeSetVariable(pb::DRAW_DECK_SIZE, IVFE{I(_draw_deck.size())});
	...
	{
		game_state_tracker().UnsafeSetVariable(pb::FIRST_PLAYER_INDEX, IVFE{0});
		auto waitingPlayersCopy = _waiting_players;
		for (auto order = pb::SECOND_PLAYER_INDEX;
				order <= pb::FORTH_PLAYER_INDEX;
				order = static_cast<pb::VariableType>(I(order) + 1)) {
			if (not waitingPlayersCopy.empty()) {
				game_state_tracker().UnsafeSetVariable(order, IVFE{waitingPlayersCopy.front()});
				waitingPlayersCopy.pop_front();
				continue;
			}
			game_state_tracker().UnsafeSetVariable(order, IVFE{-1});
		}
	}
}
...
std::optional<int> m2g::Proxy::handle_client_command(int turn_holder_index, MAYBE const m2g::pb::TurnBasedClientCommand& client_command) {
	...
	if (_is_liquidating) {
		...
		for (int i = 0; i < M2_GAME.GetTotalPlayerCount(); ++i) {
			auto money_spent_by_player_enum = static_cast<pb::VariableType>(pb::MONEY_SPENT_BY_PLAYER_0 + i);
			game_state_tracker().UnsafeSetVariable(money_spent_by_player_enum, IVFE{0});
		}
		for (auto [player_index, spent_money] : _played_players) {
			auto money_spent_by_player_enum = static_cast<pb::VariableType>(pb::MONEY_SPENT_BY_PLAYER_0 + player_index);
			game_state_tracker().UnsafeSetVariable(money_spent_by_player_enum, IVFE{spent_money});
		}
	}
	...
	if (_waiting_players.empty() && card_count == 0) {
		...
		_draw_deck.pop_back();
		game_state_tracker().UnsafeSetVariable(pb::DRAW_DECK_SIZE, IVFE{I(_draw_deck.size())});
		turn_holder_character.UnsafeAddCard(card);

		// Check if first turn finished for all players
		if (_waiting_players.empty()) {
			LOG_INFO("First turn ended");
			_is_first_turn = false;
			game_state_tracker().UnsafeSetVariable(pb::IS_LAST_ACTION_OF_PLAYER, IVFE{0});
		...
		game_state_tracker().UnsafeSetVariable(pb::IS_LAST_ACTION_OF_PLAYER, IVFE{1});
	...
	// Now that the player order is determined, fill the game state tracker with the order
	{
		...
		for (auto order = pb::FIRST_PLAYER_INDEX; order <= pb::FORTH_PLAYER_INDEX;
			...
			if (not playedPlayersCopy.empty()) {
				game_state_tracker().UnsafeSetVariable(order, IVFE{playedPlayersCopy.front().first});
...
void m2g::Proxy::bot_handle_server_update(const m2::pb::TurnBasedServerUpdate& server_update) {
	...
	if (receiver_index == server_update.turn_holder_index()) {
		...
		// Find any card
		auto card_it = std::find_if(object_it->cards().begin(), object_it->cards().end(), [](int card_type) {
...
bool m2g::Proxy::is_last_action_of_player() const {
	return game_state_tracker().GetVariable(pb::IS_LAST_ACTION_OF_PLAYER).GetIntOrZero() == 1;
}
bool m2g::Proxy::is_canal_era() const {
	return game_state_tracker().GetVariable(pb::IS_RAILROAD_ERA).GetIntOrZero() == 0;
}
bool m2g::Proxy::is_railroad_era() const {
	return game_state_tracker().GetVariable(pb::IS_RAILROAD_ERA).GetIntOrZero() == 1;
}
int m2g::Proxy::market_coal_count() const {
	return game_state_tracker().GetVariable(m2g::pb::COAL_CUBE_COUNT).GetIntOrZero();
}
int m2g::Proxy::market_iron_count() const {
	return game_state_tracker().GetVariable(m2g::pb::IRON_CUBE_COUNT).GetIntOrZero();
}
int m2g::Proxy::market_coal_cost(int coal_count) const {
	auto current_coal_count = game_state_tracker().GetVariable(m2g::pb::COAL_CUBE_COUNT).GetIntOrZero();
...
int m2g::Proxy::market_iron_cost(int iron_count) const {
	auto current_iron_count = game_state_tracker().GetVariable(m2g::pb::IRON_CUBE_COUNT).GetIntOrZero();
...
int m2g::Proxy::player_spent_money(int player_index) const {
	auto money_spent_by_player_enum = static_cast<pb::VariableType>(pb::MONEY_SPENT_BY_PLAYER_0 + player_index);
	return game_state_tracker().GetVariable(money_spent_by_player_enum).GetIntOrZero();
}
...
std::optional<std::pair<m2g::Proxy::PlayerIndex, m2g::pb::TurnBasedServerCommand>> m2g::Proxy::prepare_next_round() {
	...
	for (const auto playerId : M2_LEVEL.multiPlayerObjectIds) {
		...
		if (newPlayerMoney < 0) {
			...
			player_character.UnsafeSetVariable(pb::VICTORY_POINTS, IVFE{player_character.GetVariable(pb::VICTORY_POINTS).GetIntOrZero() + newPlayerMoney});
		}
		player_character.UnsafeSetVariable(pb::MONEY, IVFE{std::clamp(newPlayerMoney, 0, INT32_MAX)});
	}
...
m2g::Proxy::LiquidationDetails m2g::Proxy::prepare_railroad_era() {
	...
	for (const auto& merchantObjId: merchant_object_ids | std::views::values) {
		...
		if (merchantChr.HasCard(pb::CARD_CATEGORY_MERCHANT_LICENSE)) {
			merchantChr.UnsafeSetVariable(pb::BEER_BARREL_COUNT, IVFE{1});
		}
	}
	...
	game_state_tracker().UnsafeSetVariable(pb::IS_RAILROAD_ERA, IVFE{1});
	game_state_tracker().UnsafeSetVariable(pb::IS_LAST_ACTION_OF_PLAYER, IVFE{0});
...
void m2g::Proxy::buy_coal_from_market() {
	game_state_tracker().UnsafeSetVariable(pb::COAL_CUBE_COUNT, IVFE{std::max(game_state_tracker().GetVariable(pb::COAL_CUBE_COUNT).GetIntOrZero() - 1, 0)});
}

void m2g::Proxy::buy_iron_from_market() {
	game_state_tracker().UnsafeSetVariable(pb::IRON_CUBE_COUNT, IVFE{std::max(game_state_tracker().GetVariable(pb::IRON_CUBE_COUNT).GetIntOrZero() - 1, 0)});
}

void m2g::Proxy::sell_coal_to_market(int count) {
	game_state_tracker().UnsafeSetVariable(pb::COAL_CUBE_COUNT, IVFE{game_state_tracker().GetVariable(pb::COAL_CUBE_COUNT).GetIntOrZero() + count});
}

void m2g::Proxy::sell_iron_to_market(int count) {
	game_state_tracker().UnsafeSetVariable(pb::IRON_CUBE_COUNT, IVFE{game_state_tracker().GetVariable(pb::IRON_CUBE_COUNT).GetIntOrZero() + count});
}
