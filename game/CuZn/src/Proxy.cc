...
void m2g::Proxy::turnBasedServerPopulate(MAYBE const std::string& name, MAYBE const m2::pb::Level& level) {
	...
	game_state_tracker().UnsafeSetVariable(pb::DRAW_DECK_SIZE, VariableValue{I(_draw_deck.size())});
	...
	{
		game_state_tracker().UnsafeSetVariable(pb::FIRST_PLAYER_INDEX, VariableValue{0});
		...
		for (auto order = pb::SECOND_PLAYER_INDEX;
			...
			if (not waitingPlayersCopy.empty()) {
				game_state_tracker().UnsafeSetVariable(order, VariableValue{waitingPlayersCopy.front()});
				waitingPlayersCopy.pop_front();
				continue;
			}
			game_state_tracker().UnsafeSetVariable(order, VariableValue{-1});
		}
	}
}
...
std::optional<int> m2g::Proxy::handle_client_command(int turn_holder_index, MAYBE const m2g::pb::TurnBasedClientCommand& client_command) {
	...
	if (_is_liquidating) {
		...
		for (int i = 0; i < M2_GAME.GetTotalPlayerCount(); ++i) {
			...
			game_state_tracker().UnsafeSetVariable(money_spent_by_player_enum, VariableValue{0});
		}
		for (auto [player_index, spent_money] : _played_players) {
			...
			game_state_tracker().UnsafeSetVariable(money_spent_by_player_enum, VariableValue{spent_money});
		}
	}
	...
	if (_waiting_players.empty() && card_count == 0) {
		...
		if (_waiting_players.empty()) {
			...
			game_state_tracker().UnsafeSetVariable(pb::IS_LAST_ACTION_OF_PLAYER, VariableValue{0});
		...
		game_state_tracker().UnsafeSetVariable(pb::IS_LAST_ACTION_OF_PLAYER, VariableValue{1});
	...
	// Now that the player order is determined, fill the game state tracker with the order
	{
		...
		for (auto order = pb::FIRST_PLAYER_INDEX; order <= pb::FORTH_PLAYER_INDEX;
			...
			if (not playedPlayersCopy.empty()) {
				game_state_tracker().UnsafeSetVariable(order, VariableValue{playedPlayersCopy.front().first});
...
std::optional<std::pair<m2g::Proxy::PlayerIndex, m2g::pb::TurnBasedServerCommand>> m2g::Proxy::prepare_next_round() {
	...
	for (const auto playerId : M2_LEVEL.multiPlayerObjectIds) {
		...
		if (newPlayerMoney < 0) {
			...
			player_character.UnsafeSetVariable(pb::VICTORY_POINTS, VariableValue{player_character.GetVariable(pb::VICTORY_POINTS).GetIntOrZero() + newPlayerMoney});
		}
		player_character.UnsafeSetVariable(pb::MONEY, VariableValue{std::clamp(newPlayerMoney, 0, INT32_MAX)});
	}
...
m2g::Proxy::LiquidationDetails m2g::Proxy::prepare_railroad_era() {
	...
	game_state_tracker().UnsafeSetVariable(pb::IS_RAILROAD_ERA, VariableValue{1});
	game_state_tracker().UnsafeSetVariable(pb::IS_LAST_ACTION_OF_PLAYER, VariableValue{0});
...
void m2g::Proxy::buy_coal_from_market() {
	game_state_tracker().UnsafeSetVariable(pb::COAL_CUBE_COUNT, VariableValue{std::max(game_state_tracker().GetVariable(pb::COAL_CUBE_COUNT).GetIntOrZero() - 1, 0)});
}

void m2g::Proxy::buy_iron_from_market() {
	game_state_tracker().UnsafeSetVariable(pb::IRON_CUBE_COUNT, VariableValue{std::max(game_state_tracker().GetVariable(pb::IRON_CUBE_COUNT).GetIntOrZero() - 1, 0)});
}

void m2g::Proxy::sell_coal_to_market(int count) {
	game_state_tracker().UnsafeSetVariable(pb::COAL_CUBE_COUNT, VariableValue{game_state_tracker().GetVariable(pb::COAL_CUBE_COUNT).GetIntOrZero() + count});
}

void m2g::Proxy::sell_iron_to_market(int count) {
	game_state_tracker().UnsafeSetVariable(pb::IRON_CUBE_COUNT, VariableValue{game_state_tracker().GetVariable(pb::IRON_CUBE_COUNT).GetIntOrZero() + count});
}
