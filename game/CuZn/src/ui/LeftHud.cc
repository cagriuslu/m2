...
namespace {
	const auto build_button = TextBlueprint{
		...
		.onAction = [](MAYBE const Text& self) -> UiAction {
			if (auto build_prerequisite = CanPlayerAttemptToBuild(GetCharacter(M2_PLAYER.GetCharacterId()))) {
	...
	const auto develop_button = TextBlueprint{
		...
		.wrapped_font_size_in_units = 1.75f,
		.onAction = [](MAYBE const Text& self) -> UiAction {
			if (auto develop_prerequisite = CanPlayerAttemptToDevelop(GetCharacter(M2_PLAYER.GetCharacterId()))) {
	...
	const auto loan_button = TextBlueprint{
		...
		.onAction = [](MAYBE const m2::widget::Text& self) -> m2::UiAction {
			if (auto loan_prerequisite = CanPlayerAttemptToLoan(GetCharacter(M2_PLAYER.GetCharacterId()))) {
				ExecuteLoanJourney();
			} else {
	...
	const auto network_button = TextBlueprint{
		...
		.wrapped_font_size_in_units = 1.75f,
		.onAction = [](MAYBE const Text& self) -> UiAction {
			if (auto network_prerequisite = CanPlayerAttemptToNetwork(GetCharacter(M2_PLAYER.GetCharacterId()))) {
	...
	const auto scout_button = TextBlueprint{
		...
		.wrapped_font_size_in_units = 1.75f,
		.onAction = [](MAYBE const m2::widget::Text& self) -> m2::UiAction {
			if (auto scout_prerequisite = CanPlayerAttemptToScout(GetCharacter(M2_PLAYER.GetCharacterId()))) {
				ExecuteScoutJourney();
			} else {
	...
	const auto sell_button = TextBlueprint{
		...
		.wrapped_font_size_in_units = 1.75f,
		.onAction = [](MAYBE const Text& self) -> UiAction {
			if (auto sell_prerequisite = CanPlayerAttemptToSell(GetCharacter(M2_PLAYER.GetCharacterId()))) {
		...
		UiWidgetBlueprint{
			...
			.variant = TextBlueprint{
				...
				.onUpdate = [](MAYBE Text& self) {
					if (M2_GAME.IsOurTurn()) {
						if (M2G_PROXY.game_state_tracker().GetVariable(m2g::pb::IS_LAST_ACTION_OF_PLAYER).GetIntOrZero() == 1) {
