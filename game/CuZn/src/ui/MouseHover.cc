...
std::pair<UiPanelBlueprint,RectF> GenerateBuiltIndustryLocationMouseHoverUiBlueprint(const IndustryLocation loc) {
	...
	const auto industryType = ToIndustryOfFactoryCharacter(GetCharacter(factory->GetCharacterId()));
	const auto& industryCard = M2_GAME.GetCard(industryType);
	const auto industryTileType = ToIndustryTileOfFactoryCharacter(GetCharacter(factory->GetCharacterId()));
...
std::pair<UiPanelBlueprint,RectF> GenerateEmptyIndustryLocationMouseHoverUiBlueprint(const IndustryLocation loc) {
	...
	if (industries.size() == 1) {
		...
		const auto nextIndustryTileType = PlayerNextIndustryTileOfIndustry(GetCharacter(M2_PLAYER.GetCharacterId()), industries[0]);
		...
		const auto nextIndustryTileType1 = PlayerNextIndustryTileOfIndustry(GetCharacter(M2_PLAYER.GetCharacterId()), industries[0]);
		const auto nextIndustryTileType2 = PlayerNextIndustryTileOfIndustry(GetCharacter(M2_PLAYER.GetCharacterId()), industries[1]);
