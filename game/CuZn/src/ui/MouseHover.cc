...
std::pair<UiPanelBlueprint,RectF> GenerateMerchantLocationMouseHoverUiBlueprint(const MerchantLocation loc) {
	...
	const std::string merchantBenefit = merchant->GetCharacter().GetVariable(m2g::pb::BEER_BARREL_COUNT)
			? "Available" : "Exhausted";
	const auto merchantBenefitBackgroundColor = merchant->GetCharacter().GetVariable(m2g::pb::BEER_BARREL_COUNT)
