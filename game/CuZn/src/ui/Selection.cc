...
bool ask_for_confirmation(const std::string& question1, const std::string& question2, const std::string& accept_text, const std::string& decline_text) {
	...
	UiPanel::create_and_run_blocking(&blueprint, UiPanel::RelativeToWindow{RectF{0.15f, 0.15f, 0.7f, 0.7f}}, std::move(background))
...
std::optional<bool> ask_for_confirmation_with_cancellation(const std::string& question, const std::string& accept_text, const std::string& decline_text) {
	...
	auto background = M2_GAME.DrawGameToTexture(M2_LEVEL.GetCamera()->InferPositionF());
	UiPanel::create_and_run_blocking(&blueprint, UiPanel::RelativeToWindow{RectF{0.25f, 0.25f, 0.5f, 0.5f}}, std::move(background))
			.IfQuit([] { M2_GAME.quit = true; })
			.IfReturn<bool>([&](auto result) { selection = result; });
	return selection;
}
...
bool ask_for_confirmation_bottom(const std::string& question, const std::string& accept_text, const std::string& decline_text, m2::thirdparty::video::Texture background_texture) {
	...
	bool selection;
	UiPanel::create_and_run_blocking(&blueprint, UiPanel::RelativeToWindow::CreateAnchoredToPosition(RectF{M2_GAME.Dimensions().Game().ApplyRatio({0.0f, 0.9f, 1.0f, 0.1f})}), std::move(background_texture))
			.IfQuit([] { M2_GAME.quit = true; })
			.IfReturn<bool>([&](auto result) { selection = result; });
	return selection;
}
