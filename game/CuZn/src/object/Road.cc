	...
	int LinkCountOfCities(const std::set<City>& cities) {
		return std::accumulate(cities.begin(), cities.end(), 0, [](const int acc, const City city) -> int {
				...
				return acc + std::accumulate(locations.begin(), locations.end(), 0, [](const int acc2, const IndustryLocation location) -> int {
					...
					if (const auto* factory = FindFactoryAtLocation(location); factory && IsFactorySold(GetCharacter(factory->GetCharacterId()))) {
						const auto industry_tile = ToIndustryTileOfFactoryCharacter(GetCharacter(factory->GetCharacterId()));
						return acc2 + M2_GAME.GetCard(industry_tile).GetConstant(m2g::pb::LINK_BONUS).GetIntOrZero();
					}
					return acc2;
				}); });
	}
...
m2::Object* FindRoadAtLocation(const m2g::pb::SpriteType location) {
	auto roads = GetCharacterPool()
				 ...
				 | std::views::transform([](auto& chr) -> m2::Object& { return M2_LEVEL.objects[chr.GetOwnerId()]; })
...
bool IsRoadCharacter(const m2::FastCharacter& chr) {
	return M2_LEVEL.objects[chr.GetOwnerId()].GetType() == m2g::pb::ROAD;
}
