...
m2::Object* FindFactoryAtLocation(Location location) {
	auto factories = GetCharacterPool()
		...
		| std::views::transform([](const auto& chr) -> m2::Object& { return M2_LEVEL.objects[chr.GetOwnerId()]; })
...
int RequiredBeerCountToSell(IndustryLocation location) {
	if (auto* factory = FindFactoryAtLocation(location); not factory) {
		...
		auto industry_tile = ToIndustryTileOfFactoryCharacter(GetCharacter(factory->GetCharacterId()));
...
void RemoveObsoleteFactories() {
	...
	std::ranges::copy(
		...
		| std::views::transform(&m2::FastCharacter::GetOwnerId),
...
void SellFactory(m2::FastCharacter& factory_chr) {
	...
	const auto currIncomePoints = GetCharacter(M2_LEVEL.objects[factory_chr.GetOwnerId()].TryGetParent()->GetCharacterId()).GetVariable(INCOME_POINTS).GetIntOrZero();
	GetCharacter(M2_LEVEL.objects[factory_chr.GetOwnerId()].TryGetParent()->GetCharacterId()).UnsafeSetVariable(INCOME_POINTS, m2::VariableValue{ClampIncomePoints(currIncomePoints + incomeBonus)});
	// Flip the tile
	factory_chr.UnsafeSetVariable(IS_SOLD, m2::VariableValue{1});
}

bool IsFactoryCharacter(const m2::FastCharacter& chr) {
	return M2_LEVEL.objects[chr.GetOwnerId()].GetType() == m2g::pb::FACTORY;
}
...
IndustryLocation ToIndustryLocationOfFactoryCharacter(m2::FastCharacter& chr) {
	...
	return *industry_location_on_position(M2_LEVEL.objects[chr.GetOwnerId()].InferPositionF());
}
...
m2::void_expected InitFactory(m2::Object& obj, const m2::VecF& position, City city, IndustryTile industry_tile) {
	...
	_gfx.onDraw = [color](m2::Graphic& gfx) {
		...
		if (GetCharacter(gfx.GetOwner().GetCharacterId()).GetVariable(IS_SOLD).GetIntOrZero() == 1) {
		...
		// Draw the resources
		DrawResources(GetCharacter(gfx.GetOwner().GetCharacterId()));
	};
