...
m2::void_expected rpg::create_projectile(m2::Object& obj, const m2::VecF& intended_direction, const m2::Item& ranged_weapon, bool is_friend) {
	...
	const auto& sprite = M2_GAME.GetSprite(ranged_weapon.game_sprite());

	// Add physics
	auto& phy = obj.add_physique();
	auto bp = m2::box2d::example_bullet_body_blueprint();
	bp.mutable_foreground_fixture()->mutable_circ()->mutable_center_offset()->set_x(sprite.ForegroundColliderOriginToOriginVecM().x);
	bp.mutable_foreground_fixture()->mutable_circ()->mutable_center_offset()->set_y(sprite.ForegroundColliderOriginToOriginVecM().y);
	...
	chr.update = [=, &phy, &obj](m2::Character& chr) {
		if (!chr.has_resource(RESOURCE_TTL)) {
			if (is_explosive) {
				...
				phy.body = m2::box2d::create_body(*M2_LEVEL.world, obj.physique_id(), obj.position, bp);
				chr.add_named_item(M2_GAME.GetNamedItem(ITEM_AUTOMATIC_EXPLOSIVE_TTL));
