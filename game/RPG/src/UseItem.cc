...
bool rpg::UseItem(m2::Character& chr, const m2::Item& item, const float resource_multiplier) {
	// Check if costs can be paid
	if (item.GetCostCount() == 1) {
		const auto cost = item.GetCostByIndex(0);
		...
		for (size_t i = 0; i < item.GetCostCount(); ++i) {
			const auto cost = item.GetCostByIndex(i);
	...
	// Get the benefits
	for (size_t i = 0; i < item.GetBenefitCount(); ++i) {
		const auto benefit = item.GetBenefitByIndex(i);
