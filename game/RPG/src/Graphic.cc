...
void DrawAddons(const m2::Graphic& gfx, float health) {
		...
		if (m2::IsProjectionTypeParallel(M2_LEVEL.GetProjectionType())) {
			...
			dst_rect = m2::RectF{
				screen_origin_to_sprite_center_px_vec.GetX() - (m2::ToFloat(src_rect.w) * M2_GAME.Dimensions().OutputPixelsPerMeter() / m2::ToFloat(sprite.Ppm()) / 2.0f),
				screen_origin_to_sprite_center_px_vec.GetY() + (m2::ToFloat(src_rect.h) * M2_GAME.Dimensions().OutputPixelsPerMeter() * 11.0f / m2::ToFloat(sprite.Ppm()) / 2.0f / 10.0f), // Give an offset of 1.1
				M2_GAME.Dimensions().OutputPixelsPerMeter(),
				M2_GAME.Dimensions().OutputPixelsPerMeter() * 12.0f / 100.0f // 0.15 m height
		};
		} else {
			...
			if (const auto projected_addon_position = ScreenOriginToProjectionAlongCameraPlaneDstpx(addon_position)) {
				dst_rect = m2::RectF::CreateCenteredAround(*projected_addon_position, M2_GAME.Dimensions().OutputPixelsPerMeter(), M2_GAME.Dimensions().OutputPixelsPerMeter() * 12.0f / 100.0f);
			}
		}
