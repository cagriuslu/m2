...
std::optional<rpg::ChaserMode> rpg::ChaserFsm::handle_alarm_while_idle() {
	...
	if (obj->GetPhysique().GetPosition().IsNear(M2_PLAYER.GetPhysique().GetPosition(), ai->trigger_distance())) {
		// Check if path exists
		if (find_path(M2_PLAYER.GetPhysique().GetPosition(), ai->chaser().give_up_distance())) {
...
std::optional<rpg::ChaserMode> rpg::ChaserFsm::handle_hit_while_idle_or_given_up() {
	// Check if path exists
	if (find_path(M2_PLAYER.GetPhysique().GetPosition(), ai->hit_trigger_distance())) {
		return ChaserMode::Triggered;
	}
	return {};
}
...
std::optional<rpg::ChaserMode> rpg::ChaserFsm::handle_alarm_while_triggered() {
	...
	if (obj->GetPhysique().GetPosition().IsNear(M2_PLAYER.GetPhysique().GetPosition(), ai->chaser().give_up_distance())) {
		// Recalculate path to player
		find_path(M2_PLAYER.GetPhysique().GetPosition(), ai->chaser().give_up_distance());
	} else {
...
std::optional<rpg::ChaserMode> rpg::ChaserFsm::handle_alarm_while_gave_up() {
	...
	if (obj->GetPhysique().GetPosition().IsNear(M2_PLAYER.GetPhysique().GetPosition(), ai->trigger_distance())) {
		...
		if (obj->GetPhysique().GetPosition().IsNear(home_position, 1.0f)) {
...
bool rpg::ChaserFsm::find_path(const m2::VecF& target, float max_distance) {
	auto smooth_path = M2_LEVEL.pathfinder->find_grid_path(obj->GetPhysique().GetPosition(), target, m2::Float{max_distance});
...
void rpg::ChaserFsm::follow_waypoints() {
	auto closest_waypoint_it = reverse_find_first_local_min(_reverse_path, obj->GetPhysique().GetPosition());
	...
	if (next_waypoint_it != _reverse_path.crend()) {
		...
		Enemy::move_towards(*obj, target - obj->GetPhysique().GetPosition(), 30000.0f);
	} else {
