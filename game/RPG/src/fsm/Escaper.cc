...
std::optional<m2::VecF> rpg::EscaperFsm::find_direction_to_escape() {
	...
	auto angle_from_player_to_obj = (obj->GetPhysique().GetPosition() - M2_PLAYER.GetPhysique().GetPosition()).GetAngle();

	auto can_escape = [=, this](float offset) -> bool {
		auto radians_offset = angle_from_player_to_obj + offset;
		auto raycast_target = obj->GetPhysique().GetPosition() + m2::VecF::CreateUnitVectorWithAngle(radians_offset).WithLength(raycast_length);
		auto raycast_distance = m2::box2d::CheckDistance(*M2_LEVEL.world, obj->GetPhysique().GetPosition(), raycast_target, m2::thirdparty::physics::gColliderCategoryToParams[m2::I(m2::thirdparty::physics::ColliderCategory::COLLIDER_CATEGORY_OBSTACLE)].belongsTo);
