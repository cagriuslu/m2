...
std::optional<m2::VecF> rpg::EscaperFsm::find_direction_to_escape() {
	...
	auto angle_from_player_to_obj = (obj->GetPhysique().position - M2_PLAYER.GetPhysique().position).GetAngle();
	...
	auto can_escape = [=, this](float offset) -> bool {
		...
		auto raycast_distance = m2::box2d::CheckDistance(*M2_LEVEL.world[m2::I(m2g::pb::PhysicsLayer::PHYSICS_DEFAULT_LAYER)], obj->GetPhysique().position, raycast_target, m2::thirdparty::physics::gColliderCategoryToParams[m2::I(m2::thirdparty::physics::ColliderCategory::COLLIDER_CATEGORY_OBSTACLE)].belongsTo);
