...
static TextBlueprint entry_variant_1 = {
		...
		.onAction = [](MAYBE const widget::Text &self) {
			auto expect_lb = m2::pb::json_file_to_message<m2::pb::Level>(m2::GetResourceDir() / "game/MINE/levels/sp000.json");
			if (!expect_lb) {
...
static TextBlueprint entry_variant_2 = {
		.text = "NEW GAME",
		.onAction = [](MAYBE const widget::Text &self) {
			auto success = M2_GAME.LoadSinglePlayer(m2::GetResourceDir() / "game/MINE/levels/sp000.json");
			if (!success) {
