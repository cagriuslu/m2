...
namespace m2 {
	...
	public:
		explicit SplitCharacter(const ObjectId ownerId) : _ownerId(ownerId) {}
		...
		[[nodiscard]] bool HasCard(const m2g::pb::CardType ct) const { return _cards.HasCard(ct); }
		[[nodiscard]] bool HasCard(const m2g::pb::CardCategory cc) const { return _cards.HasCard(cc); }
		[[nodiscard]] int CountCards(const m2g::pb::CardType ct) const { return _cards.CountCards(ct); }
		[[nodiscard]] int CountCards(const m2g::pb::CardCategory cc) const { return _cards.CountCards(cc); }
		[[nodiscard]] m2g::pb::CardType GetFirstCardType(const m2g::pb::CardCategory cc) const { return _cards.GetFirstCardType(cc); }
		[[nodiscard]] expected<void> TryAddCard(const m2g::pb::CardType ct) { return _cards.TryAddCard(ct); }
		void UnsafeAddCard(const m2g::pb::CardType ct) { _cards.UnsafeAddCard(ct); }
		void RemoveCard(const m2g::pb::CardType ct) { _cards.RemoveCard(ct); }

		[[nodiscard]] VariableValue GetVariable(const m2g::pb::VariableType vt) const { return _variables.GetVariable(vt); }
		[[nodiscard]] bool TrySetVariable(const m2g::pb::VariableType vt, const VariableValue varVal) { return _variables.TrySetVariable(vt, varVal); }
		void UnsafeSetVariable(const m2g::pb::VariableType vt, const VariableValue varVal) { _variables.UnsafeSetVariable(vt, varVal); }
		void ClearVariable(const m2g::pb::VariableType vt) { return _variables.ClearVariable(vt); }
