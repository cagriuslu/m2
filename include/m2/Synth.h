...
namespace m2 {
	...
	namespace synth {
		...
		size_t beat_step_count(const pb::Rational& beats, int32_t max_denominator);

		Rational track_beat_count(const pb::SynthTrack& track);
		int32_t track_max_denominator(const pb::SynthTrack& track);
		...
		int32_t song_max_denominator(const pb::SynthSong& song);
