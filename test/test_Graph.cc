...
TEST(Graph, basic) {
	...
	auto generator = [&i]() -> std::optional<std::pair<m2::Graph::Node, m2::Graph::Edge>> {
		if (i < 100) {
			auto node_and_edge = std::make_pair(i, m2::Graph::Edge{i + 1, m2::FE{static_cast<float>(i) / 2.0f}});
			i++;
			return node_and_edge; // Generates 0 -> 1 -> 2 -> ... -> 98 -> 99 -> 100
		} else if (100 <= i && i < 200) {
			int j = 200 - i; // j: 100, 99, 98, 97
			i++;
			return std::make_pair(j, m2::Graph::Edge{j - 1, m2::FE{static_cast<float>(j) / 2.0f}}); // Generates 100 -> 99 -> ... -> 1 -> 0
		} else {
			return std::nullopt;
		}
	};
