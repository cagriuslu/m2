...
void m2::network::ServerThread::server_update(int destination) {
	// TODO
}
		...
		if (*select_result == 0) {
			...
			if (read_set.is_set(listen_socket->fd())) {
				...
				if (server_thread->_max_connection_count <= server_thread->_clients.size()) {
					...
					read_set.add_fd(client_socket->fd());
			...
			for (auto& client : server_thread->_clients) {
				...
				// Parse message
				std::string json_str{server_thread->_read_buffer, *recv_success};
				...
				if (not expect_message->has_client_command() && not expect_message->has_client_update()) {
					...
					if (auto sender_id = expect_message->sender_id(); sender_id) {
						...
						LOG_INFO("Client not ready", sender_id);
					}
				} else {
