...
std::string m2::to_string(const network::IpAddressAndPort& ip_port) {
	auto ip = ntohl(ip_port.first);
	auto first = (ip & 0xFF000000u) >> 24;
	auto second = (ip & 0xFF0000u) >> 16;
	auto third = (ip & 0xFF00u) >> 8;
	auto forth = (ip & 0xFFu) >> 0;
	auto port = ntohs(ip_port.second);
