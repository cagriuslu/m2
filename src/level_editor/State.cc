...
void m2::ledit::State::PaintMode::paint_sprite(const VecI& position) {
	if (!position.is_negative()) {
		...
		EraseMode::erase_position(position, std::get<ledit::State>(LEVEL.type_state).selected_layer);
		// Create/Replace placeholder
		std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(std::get<ledit::State>(LEVEL.type_state).selected_layer)][position] = std::make_pair(obj::create_background_placeholder(VecF{position}, GAME.get_sprite(selected_sprite_type), std::get<ledit::State>(LEVEL.type_state).selected_layer), selected_sprite_type);
	}
}
...
void m2::ledit::State::EraseMode::erase_position(const VecI& position) {
	erase_position(position, std::get<ledit::State>(LEVEL.type_state).selected_layer);
}
void m2::ledit::State::EraseMode::erase_position(const VecI &position, BackgroundLayer layer) {
	auto placeholders_it = std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(layer)].find(position);
	if (placeholders_it != std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(layer)].end()) {
		LEVEL.deferred_actions.push_back(create_object_deleter(placeholders_it->second.first));
		std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(layer)].erase(placeholders_it);
	}
}
...
void m2::ledit::State::RemoveMode::remove_object(const VecI &position) {
	auto placeholders_it = std::get<ledit::State>(LEVEL.type_state).fg_placeholders.find(position);
	if (placeholders_it != std::get<ledit::State>(LEVEL.type_state).fg_placeholders.end()) {
		LEVEL.deferred_actions.push_back(create_object_deleter(placeholders_it->second.first));
		std::get<ledit::State>(LEVEL.type_state).fg_placeholders.erase(placeholders_it);
	}
}
std::optional<m2g::pb::SpriteType> m2::ledit::State::PickMode::lookup_background_sprite(const VecI& position) {
	auto it = std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(std::get<ledit::State>(LEVEL.type_state).selected_layer)].find(position);
	if (it != std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(std::get<ledit::State>(LEVEL.type_state).selected_layer)].end()) {
		return it->second.second;
	}
	return {};
}
std::optional<m2::pb::LevelObject> m2::ledit::State::PickMode::lookup_foreground_object(const VecI& position) {
	auto it = std::get<ledit::State>(LEVEL.type_state).fg_placeholders.find(position);
	if (it != std::get<ledit::State>(LEVEL.type_state).fg_placeholders.end()) {
		return it->second.second;
	}
	return {};
}
...
void m2::ledit::State::SelectMode::shift_right() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		...
		for (auto& placeholders : std::get<ledit::State>(LEVEL.type_state).bg_placeholders) {
			level_editor::shift_placeholders(placeholders, LEVEL.objects, min_x, INT32_MAX, min_y, max_y, shift_count, 0);
		}
		level_editor::shift_placeholders(std::get<ledit::State>(LEVEL.type_state).fg_placeholders, LEVEL.objects, min_x, INT32_MAX, min_y, max_y, shift_count, 0);
	}
}
void m2::ledit::State::SelectMode::shift_down() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		...
		level_editor::shift_placeholders(std::get<ledit::State>(LEVEL.type_state).fg_placeholders, LEVEL.objects, min_x, max_x, min_y, INT32_MAX, 0, shift_count);
	}
}
void m2::ledit::State::SelectMode::copy() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		clipboard_layer = std::get<ledit::State>(LEVEL.type_state).selected_layer;
		auto positions = selection_result.primary_int_selection_position_m();
...
void m2::ledit::State::SelectMode::paste_bg() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		...
		if (clipboard_position_1 && clipboard_position_2) {
			clipboard_position_1->for_each_cell_in_between(*clipboard_position_2, [&](const VecI& cell) {
				auto it = std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(*clipboard_layer)].find(cell);
				if (it != std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(*clipboard_layer)].end()) {
					auto new_position = positions->first + (cell - *clipboard_position_1);
					auto sprite_type = it->second.second;
					EraseMode::erase_position(new_position, std::get<ledit::State>(LEVEL.type_state).selected_layer);
					std::get<ledit::State>(LEVEL.type_state).bg_placeholders[I(std::get<ledit::State>(LEVEL.type_state).selected_layer)][new_position] = std::make_pair(obj::create_background_placeholder(VecF{new_position}, GAME.get_sprite(sprite_type), std::get<ledit::State>(LEVEL.type_state).selected_layer), sprite_type);
				}
			});
		}
	}
...
void m2::ledit::State::SelectMode::paste_fg() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		...
		if (clipboard_position_1 && clipboard_position_2) {
			clipboard_position_1->for_each_cell_in_between(*clipboard_position_2, [&](const VecI& cell) {
				auto it = std::get<ledit::State>(LEVEL.type_state).fg_placeholders.find(cell);
				if (it != std::get<ledit::State>(LEVEL.type_state).fg_placeholders.end()) {
					auto new_position = positions->first + (cell - *clipboard_position_1);
...
void m2::ledit::State::SelectMode::erase() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		auto positions = selection_result.primary_int_selection_position_m();
		positions->first.for_each_cell_in_between(positions->second, [&](const VecI& cell) {
			ledit::State::EraseMode::erase_position(cell, std::get<ledit::State>(LEVEL.type_state).selected_layer);
		});
	}
}
...
m2::ui::Action m2::ledit::State::SelectMode::rfill() {
	if (auto selection_result = SelectionResult{GAME.events}; selection_result.is_primary_selection_finished()) {
		...
		if (ui::is_return(action) && not rfill_sprite_types.empty()) {
			positions->first.for_each_cell_in_between(positions->second, [&](const VecI& cell) {
	...
	return ui::make_continue_action();
}
void m2::ledit::State::ShiftMode::shift(MAYBE const VecI& position) const {
	if (shift_type == ShiftType::RIGHT) {
		...
		level_editor::shift_placeholders(std::get<ledit::State>(LEVEL.type_state).fg_placeholders, LEVEL.objects, position.x, INT32_MAX, INT32_MIN, INT32_MAX, 1, 0);
		...
		level_editor::shift_placeholders(std::get<ledit::State>(LEVEL.type_state).fg_placeholders, LEVEL.objects, INT32_MIN, INT32_MAX, position.y, INT32_MAX, 0, 1);
		...
		level_editor::shift_placeholders(std::get<ledit::State>(LEVEL.type_state).fg_placeholders, LEVEL.objects, position.x, INT32_MAX, position.y, INT32_MAX, 1, 1);
	}
}
