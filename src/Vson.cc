...
m2::expected<std::string> m2::Vson::query_string_value(const std::string& path) const {
	...
	m2ReturnUnexpectedUnless(vson, "VSON path not found");
	m2ReturnUnexpectedUnless(vson->is_string(), "VSON value not string");
...
m2::expected<m2::Vson> m2::Vson::parse_string(const std::string& str) {
	...
	m2ReflectUnexpected(optional_vson);
	int c;
	while ((c = ss.get()) != EOF) {
		m2ReturnUnexpectedUnless(isspace(c), "Extra characters after VSON");
	}
	return optional_vson;
}
m2::expected<m2::Vson> m2::Vson::parse_file(const std::string &fpath) {
	FILE* file = fopen(fpath.c_str(), "r");
	m2ReturnUnexpectedUnless(file, "Unable to open file");
	std::stringstream ss;
	while (not feof(file)) {
		char buffer[512];
