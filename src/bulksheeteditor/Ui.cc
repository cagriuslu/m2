				...
				.variant = widget::TextBlueprint{
					.text = "Set Rect", .onAction = [](MAYBE const widget::Text& self) -> UiAction {
						...
						if (const auto* selection = M2_LEVEL.GetPrimarySelection(); selection->IsComplete()) {
							std::get<bulksheeteditor::State>(M2_LEVEL.stateVariant).SetRect(*selectedSprite, *selection->IntegerSelectionRectM());
							std::get<bulksheeteditor::State>(M2_LEVEL.stateVariant).LookUpAndStoreSpriteRect(*selectedSprite);
						}
				...
				.variant = widget::TextBlueprint{
					.text = "Reset", .onAction = [](MAYBE const widget::Text& self) -> UiAction {
						if (const auto selectedSprite = SelectedSprite(); selectedSprite) {
							std::get<bulksheeteditor::State>(M2_LEVEL.stateVariant).Reset(*selectedSprite);
							std::get<bulksheeteditor::State>(M2_LEVEL.stateVariant).LookUpAndStoreSpriteRect(*selectedSprite);
						}
						return MakeContinueAction();
					}
					...
					.variant = widget::TextSelectionBlueprint{
						...
						.onAction = [](const widget::TextSelection& self) -> UiAction {
							if (const auto selections = self.GetSelectedOptions(); not selections.empty()) {
								...
								std::get<bulksheeteditor::State>(M2_LEVEL.stateVariant).LookUpAndStoreSpriteRect(selectedSpriteType);
								const auto sprite_name = pb::enum_name(selectedSpriteType);
								M2_LEVEL.ShowMessage(sprite_name);
							} else {
								std::get<bulksheeteditor::State>(M2_LEVEL.stateVariant).LookUpAndStoreSpriteRect(std::nullopt);
							}
							return MakeContinueAction();
						}
					}
...
const UiPanelBlueprint m2::bulksheeteditor::gMainMenu = {
	.name = "MainMenu",
    .w = 16,
    .h = 9,
    .border_width = 0,
                ...
                .onAction = [](MAYBE const widget::Text& self) -> UiAction {
	                if (const auto selections = self.Parent().FindWidget<widget::TextSelection>("ResourceSelection")->GetSelectedOptions();
		                ...
		                if (const auto spriteSheet = std::get<State>(M2_LEVEL.stateVariant).SelectResource(std::get<std::string>(selections[0]))) {
		                	M2_LEVEL.ReplaceLeftHud(std::make_unique<UiPanelBlueprint>(GenerateLeftHud(*spriteSheet)), UiPanel::RelativeToWindow::CreateAnchoredToPosition(RectF{M2_GAME.Dimensions().LeftHud()}));
	                		M2_LEVEL.ReplaceRightHud(&gRightHud, UiPanel::RelativeToWindow::CreateAnchoredToPosition(RectF{M2_GAME.Dimensions().RightHud()}));
