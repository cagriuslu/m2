...
void video::FillTriangle(Renderer& renderer, const VecF& point0, const VecF& point1, const VecF& point2, const RGBA& color) {
	...
	if (SDL_RenderGeometry(static_cast<SDL_Renderer*>(renderer.RawHandle()), nullptr, vertices, 3, nullptr, 0) < 0) {
...
void video::FillCircle(Renderer& renderer, const VecF& centerPx, const RGBA& centerColor, const float radiusPx, const RGBA& edgeColor, const unsigned steps) {
	...
	if (SDL_RenderGeometry(static_cast<SDL_Renderer*>(renderer.RawHandle()), nullptr, vertices.data(), static_cast<int>(vertices.size()), nullptr, 0) < 0) {
		throw M2_ERROR(std::string{"SDL_RenderGeometry failed: "} + SDL_GetError());
	}
}
void video::FillRectangle(Renderer& renderer, const RectF& rectPx, const RGBA& color) {
	...
	if (SDL_RenderFillRectF(static_cast<SDL_Renderer*>(renderer.RawHandle()), &rect) < 0) {
...
void video::FillRectangle(Renderer& renderer, const RectI& rectPx, const RGBA& color) {
	...
	if (SDL_RenderFillRect(static_cast<SDL_Renderer*>(renderer.RawHandle()), &rect) < 0) {
...
void video::DrawPoint(Renderer& renderer, const VecF& pointPx, const RGBA& color) {
	...
	if (SDL_RenderDrawPointF(static_cast<SDL_Renderer*>(renderer.RawHandle()),
...
void video::DrawLine(Renderer& renderer, const VecI& point0, const VecI& point1, const RGBA& color) {
	SDL_SetRenderDrawColor(static_cast<SDL_Renderer*>(renderer.RawHandle()), color.r, color.g, color.b, color.a);
	if (SDL_RenderDrawLine(static_cast<SDL_Renderer*>(renderer.RawHandle()), point0.x, point0.y, point1.x, point1.y) < 0) {
...
void video::DrawLine(Renderer& renderer, const VecF& point0, const VecF& point1, const RGBA& color) {
	SDL_SetRenderDrawColor(static_cast<SDL_Renderer*>(renderer.RawHandle()), color.r, color.g, color.b, color.a);
	if (SDL_RenderDrawLineF(static_cast<SDL_Renderer*>(renderer.RawHandle()), point0.GetX(), point0.GetY(), point1.GetX(), point1.GetY()) < 0) {
...
void video::DrawRectangle(Renderer& renderer, const RectI& rect, const RGBA& color) {
	...
	if (SDL_RenderDrawRect(static_cast<SDL_Renderer*>(renderer.RawHandle()), &sdlRect) < 0) {
...
void video::DrawRectangle(Renderer& renderer, const RectF& rect, const RGBA& color) {
	...
	if (SDL_RenderDrawRectF(static_cast<SDL_Renderer*>(renderer.RawHandle()), &sdlRect) < 0) {
		throw M2_ERROR(std::string{"SDL_RenderDrawRectF failed: "} + SDL_GetError());
	}
}
void video::DrawRectangle(Renderer& renderer, const VecF& center, const float width, const float height, const float orientationRads, const RGBA& color) {
