...
int List_Init(List* list, unsigned capacityInBitsMax16, size_t itemSize) {
	...
	M2ERR_REFLECT(Pool_Init(&list->bucket, capacityInBitsMax16, sizeof(ListItem) + itemSize));
...
void* List_InsertAfter(List* list, ID afterIterator, void* copy, ID* outIterator) {
	if (afterIterator == 0) {
		if (list->firstId == 0) {
			ID newItemId = 0;
			ListItem* newItem = static_cast<ListItem *>(Pool_Mark(&list->bucket, NULL, &newItemId));
			newItem->nextId = 0;
			newItem->prevId = 0;
			if (copy) {
			...
			ListItem* oldFirstItem = static_cast<ListItem *>(Pool_GetById(&list->bucket, oldFirstItemId));
			ID newFirstItemId = 0;
			ListItem* newFirstItem = static_cast<ListItem *>(Pool_Mark(&list->bucket, NULL, &newFirstItemId));
		...
		ListItem* iteratorItem = static_cast<ListItem *>(Pool_GetById(&list->bucket, afterIterator));
		if (!iteratorItem) {
			return NULL;
		}
		if (iteratorItem->nextId == 0) {
			ID newLastItemId = 0;
			ListItem* newLastItem = static_cast<ListItem *>(Pool_Mark(&list->bucket, NULL, &newLastItemId));
			...
			ListItem* nextItem = static_cast<ListItem *>(Pool_GetById(&list->bucket, nextItemId));
			ID newItemId = 0;
			ListItem* newItem = static_cast<ListItem *>(Pool_Mark(&list->bucket, NULL, &newItemId));
...
void* List_InsertBefore(List* list, ID beforeIterator, void* copy, ID* outIterator) {
	if (beforeIterator == 0) {
		...
		ListItem* iteratorItem = static_cast<ListItem *>(Pool_GetById(&list->bucket, beforeIterator));
		if (!iteratorItem) {
			return NULL;
		}
...
void List_Remove(List* list, ID iterator) {
	ListItem* item = static_cast<ListItem *>(Pool_GetById(&list->bucket, iterator));
	if (!item) {
		return;
	}
