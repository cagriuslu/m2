...
void_expected MessagePasser::ReadMessages(std::queue<MessageAndSender>& out) {
	...
	while (not _receivedSmallMessages.empty()) {
		...
		LOG_NETWORK_VERBOSE("Returning message from peer, with sequence number", sender, msgSequenceNo);
		// Pop from queue
...
void_expected MessagePasser::QueueMessage(MessageAndReceiver&& in) {
	...
	LOG_NETWORK_VERBOSE("Queueing outgoing message for peer, with sequence number", in.receiver, msgSequenceNo);
