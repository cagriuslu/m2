...
void UiWidget::SetRect(const RectF& rect_px) {
...
m2::RectF m2::UiWidget::calculate_wrapped_text_rect(VecF textTextureDimensions, RectF drawable_area, TextHorizontalAlignment align_h, TextVerticalAlignment align_v) {
	if (drawable_area.w < textTextureDimensions.GetX()) {
	...
	switch (align_h) {
		...
		case TextHorizontalAlignment::RIGHT:
			...
			break;
		default:
			horizontally_aligned = unaligned.AlignLeftTo(drawable_area.GetXCenter() - textTextureDimensions.GetX() / 2.0f);
			break;
	}
	...
	switch (align_v) {
		...
		default:
			return horizontally_aligned.AlignTopTo(drawable_area.GetYCenter() - textTextureDimensions.GetY() / 2.0f);
	}
}

m2::RectF m2::UiWidget::calculate_filled_text_rect(const RectF drawable_area, const TextHorizontalAlignment align, const char* text) {
	...
	const auto unaligned_destination = drawable_area.TrimToAspectRatio(defaultFontRenderedSize.GetX(), defaultFontRenderedSize.GetY());
...
void UiWidget::draw_rectangle(const RectF& rect, const RGBA& color) {
	thirdparty::video::FillRectangle(M2_GAME.GetRenderer(), rect, color);
}

void UiWidget::DrawSpriteOrTextLabel(const std::variant<Sprite, pb::TextLabel>& spriteOrTextLabel, const RectF& dst_rect) {
	RectI src_rect;
	...
	const auto actual_dst_rect = RectF{
		dst_rect.x + (dst_rect.w - (static_cast<float>(src_rect.w) * sprite_size_multiplier)) / 2,
		dst_rect.y + (dst_rect.h - (static_cast<float>(src_rect.h) * sprite_size_multiplier)) / 2,
...
void UiWidget::draw_border(const RectF& rect, const float vertical_border_width_px, const float horizontal_border_width_px, const RGBA& color) {
	thirdparty::video::FillRectangle(M2_GAME.GetRenderer(), RectF{rect.x, rect.y, vertical_border_width_px, rect.h}, color);
	thirdparty::video::FillRectangle(M2_GAME.GetRenderer(), RectF{rect.GetX2() - vertical_border_width_px, rect.y, vertical_border_width_px, rect.h}, color);
	thirdparty::video::FillRectangle(M2_GAME.GetRenderer(), RectF{rect.x, rect.y, rect.w, horizontal_border_width_px}, color);
	thirdparty::video::FillRectangle(M2_GAME.GetRenderer(), RectF{rect.x, rect.GetY2() - horizontal_border_width_px, rect.w, horizontal_border_width_px}, color);
}
