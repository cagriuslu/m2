...
void IntegerSelection::OnDraw() {
...

	{
		...
		_textTexture.first.Render(M2_GAME.GetRenderer(), _textTexture.second);
	}
	...
	const auto [plusArea, minusArea] = CalculatePlusAndMinusButtonDrawArea();
	{
		...
		if (not _plusTexture.first) {
			...
			_plusTexture.first = m2MoveOrThrowError(thirdparty::video::TextTexture::CreateNoWrap(M2_GAME.GetRenderer(), M2_GAME.font, _plusTexture.second.h, "+"));
		}
	...
	}
	{
		...
		if (not _minusTexture.first) {
			...
			_minusTexture.first = m2MoveOrThrowError(thirdparty::video::TextTexture::CreateNoWrap(M2_GAME.GetRenderer(), M2_GAME.font, _minusTexture.second.h, "-"));
		}
