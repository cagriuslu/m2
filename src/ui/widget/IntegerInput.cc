...
IntegerInput::IntegerInput(Panel* parent, const WidgetBlueprint* blueprint)
	...
	_fontTexture = m2_move_or_throw_error(sdl::FontTexture::create_nowrap(M2_GAME.renderer, M2_GAME.font, M2G_PROXY.default_font_size, to_string(_value)));
	...
	if (integer_selection_blueprint().on_create) {
		...
		if (opt_value) {
			// Save new value
			_value = *opt_value;
			_fontTexture = m2_move_or_throw_error(sdl::FontTexture::create_nowrap(M2_GAME.renderer, M2_GAME.font, M2G_PROXY.default_font_size, to_string(*opt_value)));
		}
	}
}
...
Action IntegerInput::select(int v) {
	_value = v;
	_fontTexture = std::move(*sdl::FontTexture::create_nowrap(M2_GAME.renderer, M2_GAME.font, M2G_PROXY.default_font_size, to_string(v)));
...
Action IntegerInput::on_update() {
	...
	if (pb_blueprint.on_update) {
		...
		if (optional_value) {
			...
			_fontTexture = std::move(*sdl::FontTexture::create_nowrap(M2_GAME.renderer, M2_GAME.font, M2G_PROXY.default_font_size, to_string(*optional_value)));
		}
	}
	return make_continue_action();
}
