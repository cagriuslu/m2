	...
	if (auto line_count = VariantBlueprint().line_count; line_count == 0) {
		...
		if (auto current_selection = std::ranges::find_if(_options, [](const auto& o) { return o.is_selected == true; });
			...
			if (not current_selection->text_texture_and_destination) {
				...
				auto textTexture = m2MoveOrThrowError(thirdparty::video::TextTexture::CreateNoWrap(M2_GAME.GetRenderer(), M2_GAME.font, fontSize, current_selection->blueprint_option.text));
			...
			current_selection->text_texture_and_destination->first.RenderWithColorMod(M2_GAME.GetRenderer(),
		...
		auto buttons_rect = Rect().TrimLeft(Rect().w - Rect().h / 2);
		{
			...
			_plusTexture->first.Render(M2_GAME.GetRenderer(), _plusTexture->second);
		...
		// - button
		{
			...
			_minusTexture->first.Render(M2_GAME.GetRenderer(), _minusTexture->second);
		...
		for (auto i = 0; i < VariantBlueprint().line_count; ++i) {
			if (const auto optionIndexAndTextRect = GetOptionIndexAndTextRectOfRow(i)) {
				...
				current_line.text_texture_and_destination->first.RenderWithColorMod(M2_GAME.GetRenderer(), current_line.text_texture_and_destination->second, current_line.blueprint_option.text_color);
			}
		}
		// Scroll bar
		if (VariantBlueprint().show_scroll_bar) {
			...
			// Up arrow
			{
				...
				_upArrowTexture->first.Render(M2_GAME.GetRenderer(), _upArrowTexture->second);
			...
			// Down arrow
			{
				...
				_downArrowTexture->first.Render(M2_GAME.GetRenderer(), _downArrowTexture->second);
