	...
	for (unsigned i = 0; i < HASHMAP_BUCKET_COUNT; i++) {
		...
		if (hm->itemTerm) {
			for (size_t j = 0; j < Array_Length(array); j++) {
				HashMapItem* item = static_cast<HashMapItem *>(Array_Get(array, j));
...
void* HashMap_Set(HashMap* hm, void* key, void* copy) {
	...
	if (collision) {
		...
		HashMapItem* newItem = static_cast<HashMapItem *>(Array_Append(array, NULL));
		if (newItem) {
...
void* HashMap_Get(HashMap* hm, void* key) {
	...
	for (size_t i = 0; i < array->length; i++) {
		HashMapItem* iter = static_cast<HashMapItem *>(Array_Get(array, i));
