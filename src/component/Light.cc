...
void m2::Light::DefaultDrawCallback(Light& lig) {
	...
	if (category_bits) {
		...
		box2d::Query(*M2_LEVEL.world, AABB{lig.position, 0.005f}, [&inside_body, category_bits](b2Fixture& fixture) {
		...
		if (not inside_body) {
			...
			for (unsigned i = 0; i < steps; ++i) {
				...
				auto distance = box2d::CheckDistance(*M2_LEVEL.world, lig.position, lig.position + full_span_m, category_bits);
