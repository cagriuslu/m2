...
m2::Path m2::Pathfinder::smoothen_path(const Path& reverse_path, FE max_distance_m) {
	...
	for (auto point2_it = reverse_path.begin() + 1; point2_it != reverse_path.end(); ++point2_it) {
		...
		bool eyesight = m2::box2d::CheckEyesight(*M2_LEVEL.world, m2::VecF{*point1}, m2::VecF{*point2}, thirdparty::physics::gColliderCategoryToParams[m2::I(thirdparty::physics::ColliderCategory::COLLIDER_CATEGORY_OBSTACLE)].belongsTo);
