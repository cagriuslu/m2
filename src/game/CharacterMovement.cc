...
std::pair<m2::CharacterMovementDirection, m2::VecF> m2::calculate_character_movement(Key left_key, Key right_key, Key up_key, Key down_key) {
	...
	if ((bool)left_key && M2_GAME.events.is_key_down(left_key)) {
		direction_vector.x -= 1.0f;
	}
	if ((bool)right_key && M2_GAME.events.is_key_down(right_key)) {
		direction_vector.x += 1.0f;
	}
	if ((bool)up_key && M2_GAME.events.is_key_down(up_key)) {
		direction_vector.y -= 1.0f;
	}
	if ((bool)down_key && M2_GAME.events.is_key_down(down_key)) {
		direction_vector.y += 1.0f;
	}
