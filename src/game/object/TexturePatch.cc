...
m2::Id m2::obj::create_texture_patch(const VecF& pos, SDL_Texture* texture, RectI rect, int ppm) {
	...
	gfx.on_draw = [=, src_rect = src_rect](Graphic& _gfx) {
		auto texture_width_dstpx = rect.w * GAME.dimensions().ppm / ppm;
		auto texture_height_dstpx = rect.h * GAME.dimensions().ppm / ppm;

		auto screen_origin_to_position_dstpx_vec = screen_origin_to_position_dstpx(_gfx.parent().position);
