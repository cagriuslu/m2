...
void m2::box2d::query(b2World& world, const Aabb2f& aabb, PhysiqueQueryCallback&& query_callback) {
...
void m2::box2d::query(b2World& world, const Aabb2f& aabb, FixtureQueryCallback&& query_callback) {
	FixtureQueryCallbackType q{std::move(query_callback)};
	world.QueryAABB(&q, static_cast<b2AABB>(aabb));
}

void m2::box2d::find_objects_near_position_under_mouse(Vec2f position, float max_distance, PhysiqueQueryCallback&& query_callback) {
	if (GAME.mouse_position_world_m().is_near(position, max_distance)) {
		m2::box2d::query(*LEVEL.world, m2::Aabb2f{GAME.mouse_position_world_m(), 0.0001f}, std::move(query_callback));
	}
}
