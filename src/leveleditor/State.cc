...
std::vector<m2::leveleditor::State::ForegroundObjectPlaceholderMap::const_iterator> m2::leveleditor::State::GetForegroundObjectsOfType(m2g::pb::ObjectType objType) const {
...
std::vector<m2::pb::Fixture::FixtureTypeCase> m2::leveleditor::State::GetSpriteFixtureTypes(const m2g::pb::SpriteType st) const {
	return _persistentSpriteSheets->SpriteFixtureTypes(st);
}

void m2::leveleditor::State::LoadLevelBlueprint(const pb::Level& lb) {
...
void m2::leveleditor::State::HandleMousePrimaryButton(const VecF& position) {
...
void m2::leveleditor::State::HandleMouseSecondaryButton(const VecF& position) {
...
void m2::leveleditor::State::HandleMousePrimarySelectionComplete(const VecF& firstPosition, const VecF& secondPosition) {
...
void m2::leveleditor::State::EraseBackground(const RectI& area) {
...
void m2::leveleditor::State::CopyBackground(const RectI& area) {
...
void m2::leveleditor::State::PasteBackground(const VecI& position) {
...
void m2::leveleditor::State::RandomFillBackground(const RectI& area, const std::vector<m2g::pb::SpriteType>& spriteSet) {
	area.ForEachCell([&](const VecI& pos) {
...
void m2::leveleditor::State::RemoveForegroundObject() {
...
void m2::leveleditor::State::CopyForeground() {
...
void m2::leveleditor::State::PasteForeground() {
	const auto selection = ForegroundSelectionArea();
...
int m2::leveleditor::State::AddChain(const m2g::pb::SpriteType st, const int insertIndex) {
	return _persistentSpriteSheets->AddFixtureToSprite(st, pb::Fixture::FixtureTypeCase::kChain, insertIndex);
}
void m2::leveleditor::State::RemoveFixture(const m2g::pb::SpriteType st, const int selectedIndex) {
	_persistentSpriteSheets->RemoveFixtureFromSprite(st, selectedIndex);
}
void m2::leveleditor::State::StoreWorldPoint(const int selectedIndex, const VecF& pointM) {
	const auto splitCount = M2_LEVEL.GetLeftHud()->FindWidget<widget::IntegerSelection>("CellSplitCount")->value();
...
void m2::leveleditor::State::StoreArc(int selectedIndex, const VecF& pointM, const ArcDescription& arc) {
	const auto splitCount = M2_LEVEL.GetLeftHud()->FindWidget<widget::IntegerSelection>("CellSplitCount")->value();
	const auto binnedPointM = pointM.RoundToBin(splitCount);
...
void m2::leveleditor::State::StoreTangent(const int selectedIndex, const TangentDescription& tangent) {
	const auto& state = dynamic_cast<DrawFgRightHudState&>(*M2_LEVEL.GetRightHud()->state);
	const auto& spritePb = GetSpritePb(state.SelectedObjectMainSpriteType());
	const auto& chain = spritePb.regular().fixtures(selectedIndex).chain();
...
void m2::leveleditor::State::UndoPoint(const int selectedIndex) {
	const auto& state = dynamic_cast<DrawFgRightHudState&>(*M2_LEVEL.GetRightHud()->state);
...
void m2::leveleditor::State::Draw() const {
	const auto splitCount = M2_LEVEL.GetLeftHud()->FindWidget<widget::IntegerSelection>("CellSplitCount")->value();
	...
	if (M2_LEVEL.GetRightHud()->Name() == "SelectBgRightHud") {
		...
		if (const auto selections = M2_LEVEL.GetSecondarySelection()->SelectionsM()) {
			...
			Graphic::DrawCross(point, sheeteditor::CROSS_COLOR);
		}
...
m2::void_expected m2::leveleditor::State::Save() {
	pb::Level level;
...
void m2::leveleditor::State::PaintBackground(const VecI& position, m2g::pb::SpriteType spriteType) {
...
void m2::leveleditor::State::PlaceForeground(const VecF& position, float orientation, m2g::pb::ObjectType objectType, m2g::pb::GroupType groupType, unsigned groupInstance) {
...
m2::leveleditor::State::ForegroundObjectPlaceholderMap::iterator m2::leveleditor::State::ApplySampling(const VecF& position) {
...
m2::RectF m2::leveleditor::State::ForegroundSelectionArea() const {
...
int m2::leveleditor::State::SpritePpm(ForegroundObjectPlaceholderMap::const_iterator fgObject) {
...
m2::VecF m2::leveleditor::State::WorldCoordinateToSpriteCoordinate(const ForegroundObjectPlaceholderMap::const_iterator fgObject, const VecF& worldCoordinate) {
...
void m2::leveleditor::State::StoreFixturePoint(const m2g::pb::SpriteType st, const int selectedIndex, const VecF& spriteOriginToPointVec) {
...
void m2::leveleditor::State::StoreArc(const int selectedIndex, const VecF& fromPointOffset, const VecF& toPointOffset, const float angleInRads, const int pieceCount, const bool drawTowardsRight) {
	const auto& state = dynamic_cast<DrawFgRightHudState&>(*M2_LEVEL.GetRightHud()->state);
	const auto& spritePb = GetSpritePb(state.SelectedObjectMainSpriteType());
	const auto& chain = spritePb.regular().fixtures(selectedIndex).chain();
	if (chain.points_size() == 0) {
