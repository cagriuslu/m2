...
namespace {
			...
			UiWidgetBlueprint{
				...
				.variant = TextBlueprint{
					...
					.onAction = [](const Text& self) {
						...
						return MakeReturnAction<leveleditor::ArcDescription>(leveleditor::ArcDescription{angle, pieceCount, drawTowardsRight});
					}
				}
			}
		}
				...
				UiWidgetBlueprint{
					...
					.variant = TextBlueprint{
						...
						.onAction = [](const Text& self) {
							...
							const auto pieceCount = self.Parent().FindWidget<IntegerSelection>("PieceCountInput")->value();
							return MakeReturnAction<leveleditor::TangentDescription>(leveleditor::TangentDescription{first, second, radius, pieceCount});
						}
					}
				}
			}
			...
			UiWidgetBlueprint{
				...
				.variant = TextSelectionBlueprint{
					...
					.onCreate = [](TextSelection& self) {
						...
						const auto& objectSet = std::get<leveleditor::State>(M2_LEVEL.stateVariant).physicsObjectsToDraw;
					...
					.onAction = [](const TextSelection& self) -> UiAction {
						auto& objectSet = std::get<leveleditor::State>(M2_LEVEL.stateVariant).physicsObjectsToDraw;
						objectSet.clear();
	...
	const UiPanelBlueprint gPaintBgRightHud = {
		...
		.onDestroy = [] {
			...
			if (std::holds_alternative<leveleditor::State>(M2_LEVEL.stateVariant)) {
				// Delete ghost
				if (const auto& levelEditorState = std::get<leveleditor::State>(M2_LEVEL.stateVariant); levelEditorState.ghostId) {
			...
			UiWidgetBlueprint{
				...
				.variant = TextSelectionBlueprint{
					...
					.onAction = [](const TextSelection& self) {
						// Create ghost
						auto& levelEditorState = std::get<leveleditor::State>(M2_LEVEL.stateVariant);
		...
		.widgets = {
				...
				.variant = TextBlueprint{
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (const auto integerSelection = M2_LEVEL.GetPrimarySelection()->IntegerSelectionRectM()) {
							std::get<leveleditor::State>(M2_LEVEL.stateVariant).CopyBackground(*integerSelection);
						}
						return MakeContinueAction();
					}
				}
			...
			UiWidgetBlueprint{
				...
				.variant = TextBlueprint{
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (const auto integerSelection = M2_LEVEL.GetPrimarySelection()->IntegerSelectionRectM()) {
							std::get<leveleditor::State>(M2_LEVEL.stateVariant).PasteBackground(integerSelection->GetTopLeftPoint());
						}
						return MakeContinueAction();
					}
				}
			...
			UiWidgetBlueprint{
				...
				.variant = TextBlueprint{
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (const auto integerSelection = M2_LEVEL.GetPrimarySelection()->IntegerSelectionRectM()) {
							std::get<leveleditor::State>(M2_LEVEL.stateVariant).EraseBackground(*integerSelection);
						}
						return MakeContinueAction();
					}
				}
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (const auto* selection = M2_LEVEL.GetPrimarySelection(); selection->IsComplete()) {
							...
							(void) action.IfReturn<std::vector<m2g::pb::SpriteType>>([&](const auto& selectedSprites) {
								std::get<leveleditor::State>(M2_LEVEL.stateVariant).RandomFillBackground(*area, selectedSprites);
							});
						}
						return MakeContinueAction();
					}
					...
					.onAction = [](const Text&) -> UiAction {
						(void) UiPanel::create_and_run_blocking(&gCoordinateDialog, UiPanel::RelativeToWindow{RectF{0.25f, 0.4f, 0.5f, 0.3f}})
								.IfReturn<VecF>([](const auto& vec) {
									std::get<leveleditor::State>(M2_LEVEL.stateVariant).HandleMouseSecondaryButton(vec);
								});
						return MakeContinueAction();
					}
				}
		...
		.widgets = {
				...
				.variant = TextBlueprint{
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (M2_LEVEL.GetPrimarySelection()->IsComplete()) {
							std::get<leveleditor::State>(M2_LEVEL.stateVariant).CopyForeground();
						}
						return MakeContinueAction();
					}
				}
			...
			UiWidgetBlueprint{
				...
				.variant = TextBlueprint{
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (M2_LEVEL.GetPrimarySelection()->IsComplete()) {
							std::get<leveleditor::State>(M2_LEVEL.stateVariant).PasteForeground();
						}
						return MakeContinueAction();
					}
				}
			...
			UiWidgetBlueprint{
				...
				.variant = TextBlueprint{
					...
					.onAction = [](MAYBE const Text& self) -> UiAction {
						if (M2_LEVEL.GetPrimarySelection()->IsComplete()) {
							std::get<leveleditor::State>(M2_LEVEL.stateVariant).RemoveForegroundObject();
						}
						return MakeContinueAction();
					}
				}
			...
			UiWidgetBlueprint{
				...
				.variant = TextSelectionBlueprint{
					...
					.onHover = [](const TextSelection& self, const std::optional<int> indexUnderMouse) {
						if (indexUnderMouse) {
							const auto& leditState = std::get<leveleditor::State>(M2_LEVEL.stateVariant);
							const auto& drawFgState = dynamic_cast<leveleditor::DrawFgRightHudState&>(*self.Parent().state);
					...
					.onUpdate = [](TextSelection& self) -> UiAction {
						const auto& leditState = std::get<leveleditor::State>(M2_LEVEL.stateVariant);
						if (const auto& state = dynamic_cast<leveleditor::DrawFgRightHudState&>(*self.Parent().state);
							...
							std::ranges::transform(leditState.GetSpritePb(state.SelectedObjectMainSpriteType()).regular().fixtures(), std::back_inserter(options),
								[](const auto& fixture) -> TextSelectionBlueprint::Option {
									auto line = sheeteditor::gFixtureTypeNames.at(fixture.fixture_type_case()) + "(" + fixture.name() + ")";
			...
			UiWidgetBlueprint{
				...
				.variant = TextBlueprint{
					...
					.onAction = [](const Text& self) -> UiAction {
						...
						auto& leditState = std::get<leveleditor::State>(M2_LEVEL.stateVariant);
						const auto& state = dynamic_cast<leveleditor::DrawFgRightHudState&>(*self.Parent().state);
					...
					.onAction = [](const Text& self) -> UiAction {
						if (const auto* selection = M2_LEVEL.GetSecondarySelection(); selection->IsComplete()) {
							...
							if (const auto selectedIndexes = fixtureSelectionWidget->GetSelectedIndexes(); not selectedIndexes.empty()) {
								const auto selectedIndex = selectedIndexes[0];
								const auto& state = dynamic_cast<leveleditor::DrawFgRightHudState&>(*self.Parent().state);
								if (const auto fixtureType = std::get<leveleditor::State>(M2_LEVEL.stateVariant).GetSpriteFixtureTypes(state.SelectedObjectMainSpriteType())[selectedIndex];
										fixtureType == pb::Fixture::FixtureTypeCase::kChain) {
									std::get<leveleditor::State>(M2_LEVEL.stateVariant).StoreWorldPoint(selectedIndex, selection->SelectionsM()->first);
								}
							}
						}
						return MakeContinueAction();
							...
							if (const auto selectedIndexes = fixtureSelectionWidget->GetSelectedIndexes(); not selectedIndexes.empty()) {
								...
								if (const auto fixtureType = std::get<leveleditor::State>(M2_LEVEL.stateVariant).GetSpriteFixtureTypes(state.SelectedObjectMainSpriteType())[selectedIndex];
									...
									UiPanel::create_and_run_blocking(&gAngleDialog, UiPanel::RelativeToWindow{RectF{0.27f, 0.37f, 0.46f, 0.26f}})
										.IfReturn<leveleditor::ArcDescription>([=](const auto& arcDesc) {
											std::get<leveleditor::State>(M2_LEVEL.stateVariant).StoreArc(selectedIndex, point, arcDesc);
										});
								}
							}
						}
								...
								if (const auto pointCount = leditState.GetSpritePb(drawFgState.SelectedObjectMainSpriteType()).regular().fixtures(selectedIndex).chain().points_size(); 1 < pointCount) {
									UiPanel::create_and_run_blocking(
										std::make_unique<UiPanelBlueprint>(CreateDistinctIntegerSelectionDialog(0, pointCount - 1)), UiPanel::RelativeToWindow{RectF{0.27f, 0.37f, 0.46f, 0.26f}})
											.IfReturn<leveleditor::TangentDescription>([=](const auto& tangent) {
												std::get<leveleditor::State>(M2_LEVEL.stateVariant).StoreTangent(selectedIndex, tangent);
											});
								}
							}
						}
				...
				.variant = TextBlueprint{
					...
					.onAction = [](const Text& self) -> UiAction {
						...
						if (const auto selectedIndexes = fixtureSelectionWidget->GetSelectedIndexes(); not selectedIndexes.empty()) {
							...
							if (const auto fixtureType = std::get<leveleditor::State>(M2_LEVEL.stateVariant).GetSpriteFixtureTypes(state.SelectedObjectMainSpriteType())[selectedIndex];
									fixtureType == pb::Fixture::FixtureTypeCase::kChain) {
								std::get<leveleditor::State>(M2_LEVEL.stateVariant).UndoPoint(selectedIndex);
							}
						}
						return MakeContinueAction();
					}
...
const UiPanelBlueprint leveleditor::gLeftHudBlueprint = {
	.name = "LeftHud",
    .w = 19, .h = 72,
    .background_color = {25, 25, 25, 255},
...
const UiPanelBlueprint leveleditor::gRightHudBlueprint = {
		.name = "RightHud",
	    .w = 19, .h = 72,
		.background_color = {25, 25, 25, 255}};
